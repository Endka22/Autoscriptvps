...
read -rp "User: " -e user
egrep -w "^### Vless $user" /etc/nginx/conf.d/v2ray.conf >/dev/null
if [ $? -eq 0 ]; then
...
END
sed -i '$ i### Vless '"$user"' '"$exp"'' /etc/nginx/conf.d/v2ray.conf
sed -i '$ ilocation /vless@'"$user"'' /etc/nginx/conf.d/v2ray.conf
...
sed -i '$ i}' /etc/nginx/conf.d/v2ray.conf
vlesslink1="vless://${uuid}@${domain}:443?path=/vless@${user}&security=tls&encryption=none&type=ws#${user}"
vlesslink2="vless://${uuid}@${domain}:80?path=/vless@${user}&encryption=none&type=ws#${user}"
